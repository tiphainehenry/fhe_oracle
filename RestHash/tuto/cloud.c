...
int main() {
    ...
    FILE* cloud_data1 = fopen("1.cloud.data","rb");
    for (int i=0; i<16; i++) import_gate_bootstrapping_ciphertext_fromFile(cloud_data1, &ciphertext1[i], params);
    fclose(cloud_data1);

    FILE* cloud_data2 = fopen("2.cloud.data","rb");
    for (int i=0; i<16; i++) import_gate_bootstrapping_ciphertext_fromFile(cloud_data2, &ciphertext2[i], params);
    fclose(cloud_data2);
